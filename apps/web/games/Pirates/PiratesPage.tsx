...
import RandzuCoverSVG from '@/public/image/games/soon.svg';
import { api } from '@/trpc/react';
import { getEnvContext } from '@/lib/envContext';
import { DEFAULT_PARTICIPATION_FEE } from 'zknoid-chain-dev/dist/src/engine/LobbyManager';
import { MOVE_TIMEOUT_IN_BLOCKS } from 'zknoid-chain-dev/dist/src/engine/MatchMaker';
import RandzuCoverMobileSVG from '@/public/image/games/soon.svg';
...
export default function PiratesPage({
  ...
  const collectPending = async () => {
    ...
    const tx = await client.transaction(
      ...
      async () => {
        piratesLogic.collectPendingBalance();
      }
    );
  ...
  const onCellClicked = async (x: number, y: number) => {
    ...
    const updatePiratesField = RandzuField.from(updatedField);

    const winWitness1 = updatePiratesField.checkWin(currentUserId);

    const tx = await client.transaction(
      sessionPrivateKey.toPublicKey(),
      async () => {
        piratesLogic.makeMove(
          UInt64.from(matchQueue.gameInfo!.gameId),
          updatePiratesField,
          winWitness1 ??
            new WinWitness(
        ...
        >
          {networkStore.address ? (
            <>
              {!competition ? (
                <GameWrap>
                  <UnsetCompetitionPopup gameId={piratesConfig.id} />
                </GameWrap>
              ) : (
                <>
                  {gameState == GameState.Won &&
                    (isRateGame &&
                    !rateGameStore.ratedGamesIds.includes(piratesConfig.id) ? (
                      <GameWrap>
                        <RateGame
                          gameId={piratesConfig.id}
        ...
        <Competition
          ...
          competition={
            competition && {
              ...
              game: {
                id: piratesConfig.id,
                name: piratesConfig.name,
                rules: piratesConfig.rules,
